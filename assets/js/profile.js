...
function renderMemoryWall(){
  ...
  box.innerHTML = years.map(year => {
    ...
    return `
      <section class="memory-year">
        <div class="memory-year-head"><h3>${year}</h3><span>${year === TK_CURRENT_YEAR ? "Hiện tại" : "Ký ức"}</span></div>
        <div class="memory-month-grid">${months.map(month => renderMonthCard(year, month)).join("")}</div>
      </section>`;
  }).join("");
}
...
function renderMonthCard(year, month){
  ...
  return `
    <article class="memory-month-card ${posts.length ? "has-memory" : "is-empty"}">
      <div class="memory-thumb">${img ? `<img src="${asset(img)}" alt="${year} ${monthLabel(month)}">` : `<span>✦</span>`}</div>
      <div class="memory-month-info"><b>${monthLabel(month)}</b><em>${count ? `${count} ảnh/kỷ niệm` : "Chưa cập nhật"}</em></div>
    </article>`;
}
...
function renderTimeline(){
  ...
  box.innerHTML = Object.keys(grouped).sort((a,b) => Number(b)-Number(a)).map(year => `
    <section class="timeline-year"><h3>${year}</h3><div class="timeline-list">${grouped[year].map(renderPostCard).join("")}</div></section>
  `).join("");
}
...
function renderPostCard(post){
  return `
    <article class="profile-post-card">
      ${post.cover_image ? `<img src="${asset(post.cover_image)}" alt="${post.title || ""}" data-lightbox="${asset(post.cover_image)}">` : ""}
      <div><span>${String(post.date || "").split("-").reverse().join("/")}</span><h4>${post.title || "Kỷ niệm"}</h4><p>${post.content || ""}</p></div>
    </article>`;
}
