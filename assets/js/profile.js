...
function tkProfileSavedUser(){try{return JSON.parse(localStorage.getItem("tk_current_user")||localStorage.getItem("tk_user")||"null");}catch(e){return null;}}
function canContribute(){const u=tkProfileSavedUser();const role=String(u?.role||"").toLowerCase();const name=String(u?.username||u?.phone||"");return role==="vip"||role==="family"||name==="0947924444";}
function fileToDataUrl(file){return new Promise((resolve,reject)=>{const r=new FileReader();r.onload=()=>resolve(r.result);r.onerror=reject;r.readAsDataURL(file);});}
async function setupProfileContribution(){const box=document.getElementById("profileContributeBox");if(!box)return;if(canContribute())box.classList.remove("hidden");const form=document.getElementById("profilePostForm");if(!form)return;const date=document.getElementById("postDate");if(date)date.value=new Date().toISOString().slice(0,10);form.onsubmit=async e=>{e.preventDefault();const files=[...(document.getElementById("postImages").files||[])].filter(f=>/^image\//.test(f.type)).slice(0,5);const images=await Promise.all(files.map(fileToDataUrl));const d=document.getElementById("postDate").value||new Date().toISOString().slice(0,10);const post={id:"local_"+Date.now(),title:document.getElementById("postTitle").value||"Kỷ niệm mới",date:d,content:document.getElementById("postContent").value||"",visibility:document.getElementById("postVisibility").value||"public",status:"published",cover_image:images[0]||"",images};const list=JSON.parse(localStorage.getItem("tk_profile_posts")||"[]");list.unshift(post);localStorage.setItem("tk_profile_posts",JSON.stringify(list));alert("Đã thêm vào Timeline.");location.reload();};}
document.addEventListener("DOMContentLoaded", setupProfileContribution);
